                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: const InputDecoration(
                          labelText: 'Franchise Login Email',
                        ),
                        validator: (v) {
                          ...
                          final value = v?.trim() ?? '';
                          if (value.isEmpty) {
                            return 'Required at branch creation';
                          }
                          if (!value.contains('@')) {
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: isEdit
                              ? 'Branch Password (leave blank to keep current)'
                              : 'Branch Password',
                        ),
                        obscureText: true,
                        validator: (v) {
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  final branchLoginEmail = loginEmailController.text
                      .trim()
                      .toLowerCase();
                  final branchPassword = passwordController.text.trim();
                  final ownerDisplayName = ownerController.text.trim().isEmpty
                  ...
                  final toSave = Branch(
                    ...
                    royaltyPercentage: double.parse(
                      royaltyController.text.trim(),
                    ),
                    status: selectedStatus,
                    loginEmail: branchLoginEmail.isEmpty
                        ? null
                        : branchLoginEmail,
                    authUid: branch?.authUid,
                    mainBranchId: branch?.mainBranchId,
                    createdAt: branch?.createdAt ?? DateTime.now(),
                    createdBy:
                        branch?.createdBy ?? currentUser?.uid ?? 'super_admin',
                  );

                  try {
                    final authService = ref.read(firebaseAuthServiceProvider);

                    String successMessage = 'Branch saved successfully.';

                    if (!isEdit &&
                        branchLoginEmail.isNotEmpty &&
                        branchPassword.isNotEmpty) {
                      final authResult = await authService
                          .createFranchiseAccount(
                            email: branchLoginEmail,
                            password: branchPassword,
                            displayName: ownerDisplayName,
                          );

                      final saved = toSave.copyWith(
                      ...
                      await authService.upsertFranchiseUserProfile(
                      ...
                      successMessage =
                          'Franchise created! Login: $branchLoginEmail';
                    } else if (isEdit) {
                      await ref
                          .read(branchRepositoryProvider)
                    ...
                    // Pop dialog first, then show snackbar via dialogContext
                    // (dialogContext stays valid until pop completes).
